...
// run invokes the probe function and records the result. It returs the probe
...
func (p *Prober) RunHandler(w http.ResponseWriter, r *http.Request) error {
	...
	return tsweb.Error(http.StatusOK, fmt.Sprintf("Probe succeeded in %v\n%s", info.Latency, stats), nil)
}
